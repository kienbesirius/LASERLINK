...
class LASERLINKAPP(tk.Tk):
    def __init__(self):
        ...
        self._hscan_programmatic: bool = False
    ...
    def flow_core(self, *, mo: str, h: str, model: str, needpsn: str) -> None:
        ...
        try:
            ...
            # worker end marker is always DONE event (flow_core cũng sẽ put DONE)
            # Re-enable background listener after flow is done.
            self.open_flow_core_resume(reason="flow_done")
    ...
    def open_flow_core_start(self) -> None:
        """Start background listener that reads COM_LASER and auto-fills H scan."""
    ...
    def _open_flow_core_loop(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                self.open_flow_core_pause(reason="H_detected")
                try:
        ...
        try:
            while True:
                ...
                if kind == "ERR":
                    ...
                    self._inject_h_from_open_flow(h)
    ...
    def _inject_h_from_open_flow(self, h: str) -> None:
        """Fill H scan entry from COM_LASER listener, then start the existing flow."""
        try:
            if getattr(self, "_hscan_is_placeholder", False):
                self._hscan_is_placeholder = False
                try:
        ...
        try:
            self._hscan_programmatic = True
            self._v_h_scan.set(h)
        finally:
            self._hscan_programmatic = False
